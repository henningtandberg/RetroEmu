...
namespace RetroEmu.Gui.Widgets.MemoryDebugger;

public class MemoryEditorWidget(IDebugInternalRam internalRam, IDebugCartridge cartridge) : IGuiWidget
{
    ...
    public void Draw(GameTime gameTime)
    {
        ...
        if (ImGui.BeginTabBar("RAM Tab Bar"))
        {
            if (ImGui.BeginTabItem("RAM"))
            {
                var ram = internalRam.GetWorkRam();
                Draw("RAM", ram, ram.Length);
                ImGui.EndTabItem();
            }
          
            if (ImGui.BeginTabItem("HRAM"))
            {
                var hRam = internalRam.GetHighRam();
                Draw("HRAM", hRam, hRam.Length);
                ImGui.EndTabItem();
            }
            
            var romBanks = cartridge.GetRomBanks();
            if (romBanks.Count > 0 && ImGui.BeginTabItem("ROM 00"))
            {
                var rom00 = romBanks.First();
                Draw("ROM 00", rom00, rom00.Length);
                ImGui.EndTabItem();
            }
...
internal class ImGuiListClipper2
{
    ...
    public unsafe void Begin(int count, float items_height = -1.0f)
    {
        ...
        if (ItemsHeight > 0.0f)
        {
            int dispStart = 0, dispEnd = 0xFFFF;
